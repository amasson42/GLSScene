...
namespace GLS {
    ...
    std::shared_ptr<Shader> Shader::standardFragmentMesh() {
        ...
        "        float shadow = 1.0;\n"
        "        if (light.caster_index >= 0) {\n"
        "            vec4 fpos_light = light_casters[light.caster_index].vp * vec4(fin_wposition, 1.0);\n"
        "            vec3 proj_coords = fpos_light.xyz / fpos_light.w;\n"
        "            proj_coords = proj_coords * 0.5 + 0.5;\n"
        "            float current_depth = proj_coords.z;\n"
        "            float bias = 0.0005;\n"// max(0.05 * (1.0 - dot(normal_direction, -light.direction)), 0.005);\n"
        "            shadow += 1.0;\n"
        "            vec2 texel_size = 1.0 / textureSize(light_casters[light.caster_index].depth_map, 0);\n"
        "            for (int x = -1; x <= 1; x++)\n"
        "                for (int y = -1; y <= 1; y++) {\n"
        "                    float pcf_depth = texture(light_casters[light.caster_index].depth_map, proj_coords.xy + vec2(x, y) * texel_size).r;\n"
        ...
        "                   specular_color += diffuse_intensity * light.specular * specular * specular_intensity * shadow;\n"
        "               }\n"
        "               break;\n"
        "           case 2:\n" // spotlight
