...
namespace GLS {
    ...
    std::shared_ptr<Shader> Shader::standardFragmentMesh() {
        ...
        "               light_color = light.color * diffuse * diffuse_intensity * attenuation;\n"
        "               diffuse_color += light_color;\n"
        "               if (diffuse_intensity > 0) {\n"
        "                   vec3 reflection = reflect(-light_direction, normal_direction);\n"
        "                   float specular_intensity = pow(clamp(dot(fcamera_position, reflection), 0.0, 1.0), shininess);\n"
        ...
        "               float spot_result = dot(light_direction, light_direction);\n"
